...
void Scene::paintEvent(QPaintEvent *event) {
    ...
    for (auto i: worker->questions) {
        Answer *it = i.getAnswer();
        painter.setBrush(QBrush(it->color, Qt::SolidPattern));
        painter.drawEllipse(QPointF(it->get_x_position(), it->get_y_position()), 2 * it->get_radius(),
                            2 * it->get_radius());
        painter.drawText(
                QPoint(it->get_x_position() - it->get_radius() / 4, it->get_y_position() + it->get_radius() / 4),
                QString::number(it->get_number()));
    }
    ...
    painter.drawEllipse(QPointF(worker->player.get_x_position(), worker->player.get_y_position()),
                        2 * worker->player.get_radius(), 2 * worker->player.get_radius());

    fnt.setPixelSize(40);
    painter.setFont(fnt);
    painter.drawText(QPoint(1700, 40), worker->text);
    painter.drawText(QPoint(1820, 40), QString::number(worker->score));
    painter.drawText(QPoint(800, 40),
                     QString::fromLocal8Bit(worker->questions[worker->currQuestion].getQuestion().c_str()));
